...
static float calculateLabelForData(const std::vector<DataElem>& data) {
...
static float calculateClassificationError(float label, const std::vector<DataElem>& data) {
...
static float calculateClassificationError(const std::vector<DataElem>& data) {
    return calculateClassificationError(calculateLabelForData(data), data);
}

std::shared_ptr<DecisionTree> DecisionTree::greedyTrain(const std::vector<DataElem>& training_data) {
    std::shared_ptr<DecisionTree> tree(new DecisionTree);
    tree->root_ = greedyTrain(training_data, nullptr, 0);
    return tree;
}

std::shared_ptr<DecisionTreeNode> DecisionTree::greedyTrain(const std::vector<DataElem>& training_data, std::shared_ptr<DecisionTreeNode> parent, uint64_t node_number) {
    ...
    std::shared_ptr<std::vector<DataElem>> best_left_group;
    std::shared_ptr<std::vector<DataElem>> best_right_group;
    for (auto feature : training_data[0].features) {
        std::string feature_key = feature.first;
        for (float split = 0.0; split < 1.0; split += 0.10) {
            auto left_group = std::make_shared<std::vector<DataElem>>();
            auto right_group = std::make_shared<std::vector<DataElem>>();
            for (auto elem : training_data) {
                if (elem.features[feature_key] < split) {
...
float DecisionTree::testAccuracy(const std::vector<DataElem>& testing_data, bool verbose) {
